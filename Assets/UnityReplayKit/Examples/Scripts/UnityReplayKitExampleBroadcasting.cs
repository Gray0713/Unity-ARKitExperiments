...
namespace Rarebyte.REK.Examples {
  ...
  public class UnityReplayKitExampleBroadcasting : MonoBehaviour {
    ...
    public void Start() {
      ...
      UnityReplayKit.Instance.BroadcastServiceSelectionFailed += (error) => Debug.LogError("Broadcast selection failed: " + error.ToString());
      UnityReplayKit.Instance.BroadcastStarted += () => Debug.Log("Broadcast started");
      UnityReplayKit.Instance.BroadcastStartFailed += (error) => Debug.LogError("Broadcast failed to start: " + error.ToString());
      UnityReplayKit.Instance.BroadcastStopped += () => Debug.Log("Broadcast stopped successfully");
      UnityReplayKit.Instance.BroadcastStopFailed += (error) => Debug.LogError("Broadcast failed to stop: " + error.ToString());
      UnityReplayKit.Instance.BroadcastFinished += () => Debug.Log("Broadcast finished");
      UnityReplayKit.Instance.BroadcastFinishFailed += (error) => Debug.LogError("Broadcast failed to finish: " + error.ToString());
    }
